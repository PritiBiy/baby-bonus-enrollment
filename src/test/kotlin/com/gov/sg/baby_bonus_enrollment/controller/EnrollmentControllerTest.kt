...
class EnrollmentControllerTest {
    ...
    inner class GetById {
        ...
        fun `returns 200 with masked NRICs and disbursement for a known enrollment ID`() {
            ...
            given(icaClient.findChild(eq("T2400005E"))).willReturn(
                ChildRecord("T2400005E", "Tan Baby Get", LocalDate.of(2025, 5, 1), Citizenship.SINGAPORE_CITIZEN)
            )
            given(iroasClient.findParent(eq("S8001234A"))).willReturn(
                ParentRecord("S8001234A", "Tan Ah Kow")
            )
            ...
            val postResult = mockMvc.perform(
                post("/api/v1/enrollments")
                    .contentType(MediaType.APPLICATION_JSON)
                    .header("X-API-Key", API_KEY)
                    .content("""{"childNric": "T2400005E", "parentNric": "S8001234A", "relationship": "FATHER"}""")
            ).andExpect(status().isCreated).andReturn()

            val created = objectMapper.readValue<EnrollmentResponse>(postResult.response.contentAsString)

            val result = mockMvc.perform(
                get("/api/v1/enrollments/${created.id}")
                    .header("X-API-Key", API_KEY)
        ...
        fun `returns 404 for an unknown enrollment ID`() {
            val result = mockMvc.perform(
                get("/api/v1/enrollments/${UUID.randomUUID()}")
                    .header("X-API-Key", API_KEY)
